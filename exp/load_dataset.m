...
%         load('20newsgroups_Count.mat'); X = X.'; % 961 x 10094, condition ~3e3
    ...
    case '20newsgroups_tfidf'
        % Check if the dataset is available locally
        if exist('20newsgroups_Tfidf.mat','file') ~= 2
            error( '\nMISSING DATASET!\n Please run the python notebook 20newsgoups_vectorized.ipynb first.')
        end
        
        load('20newsgroups_Tfidf.mat'); % 961 x 10094 sparse, condition ~1e2
        % Same as Fercoq et al 2015 'Mind the duality gap' :
        % dataset obtained with bag of words features extracted from the 
        % 20newsgroup dataset (comp.graphics vs. talk.religion.misc with 
        % TF-IDF removing English stop words and words occurring only once 
        % or more than 95% of the time). Text feature extraction was done using Scikit-Learn
...
%         clear T
    ...
    case 'rcv1'
        ...
        if exist('rcv1_rankdef.mat','file') ~= 2
            error( ['\nMISSING DATASET!\n', ...
                   'Please download the RCV1 dataset at LIBSVM:\n' ...
...
%         names = names(index',:);
        ...
        if strcmp(exp_type,'Cuprite_USGS-lib')
            ...
            if normalizeA, Y_orig = Y_orig./repmat(sqrt(sum(Y_orig.^2)),mc_it,1); end
...
if exist('normalizeA','var') && normalizeA
...
%Reverse n-m notation
aux = n; n = m; m = aux; clear aux
