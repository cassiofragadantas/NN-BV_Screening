...
%    w = C'*resid;
   % With screening
   resid = d - C(:,P)*x(P); %MODIF! restricting to active set;
%    w(~screen_vec) = C(:,~screen_vec).'*resid; %MODIF! restricting to non-screened set (slower)
   w(~screen_vec) = Cscr'*resid; %MODIF! restricting to non-screened set
   
   %% Safe screening
   if mod(outeriter,screen_period) == 0
       screen_prev = screen_vec;
       % Notation: d - C*X
       [screen_new, ~] = nnGapSafeScreen(d, C, resid, w, normC, sumC, options.tdual);
       screen_vec = screen_new | screen_prev;
       x(screen_vec) = 0; w(screen_vec) = 0;
       
%        Cscr = C(:,~screen_vec); % slower
       screen_new(screen_prev) = [];
       Cscr(:,screen_new) = []; 
...
output.time_it = output.time_it(1:outeriter);
output.nb_screen_it = output.nb_screen_it(1:outeriter);
if calc_gap, output.gap_it= output.gap_it(1:outeriter); end
